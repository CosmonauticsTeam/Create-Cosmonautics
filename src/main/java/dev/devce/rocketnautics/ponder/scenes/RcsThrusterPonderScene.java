...
public final class RcsThrusterPonderScene {
	...
	public static void show(SceneBuilder scene, SceneBuildingUtil util) {
		...
		scene.idle(120);
		setCasing(scene, rcs, RCSThrusterBlock.Casing.BRASS);
		showText(scene, util, "text_5", rcs);
		scene.idle(120);
		setCasing(scene, rcs, RCSThrusterBlock.Casing.COPPER);
		showText(scene, util, "text_6", rcs);
		scene.idle(120);
		setCasing(scene, rcs, RCSThrusterBlock.Casing.RAILWAY);
	...
	private static void setCasing(SceneBuilder s, BlockPos pos, RCSThrusterBlock.Casing casing) {
		s.world().setBlock(pos, RocketBlocks.RCS_THRUSTER.getDefaultState()
				.setValue(net.minecraft.world.level.block.DirectionalBlock.FACING, Direction.WEST)
				.setValue(RCSThrusterBlock.CASING, casing), false);
	...
	private static void showText(SceneBuilder s, SceneBuildingUtil u, String key, BlockPos pos) {
		s.overlay().showText(100).text("rocketnautics.ponder.rcs_thruster." + key).pointAt(u.vector().topOf(pos))
				.placeNearTarget();
	}
}
